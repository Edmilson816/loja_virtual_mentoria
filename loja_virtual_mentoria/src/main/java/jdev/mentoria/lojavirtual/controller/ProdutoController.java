...
public class ProdutoController {
	...
	public ResponseEntity<Produto> salvarAcesso(@RequestBody @Valid Produto produto) throws ExceptionMentoriaJava, MessagingException, IOException { /*Recebe o JSON e converte pra Objeto*/
		...
		if (produto.getNome().length() < 10) {
			throw new ExceptionMentoriaJava("Nome do produto deve conter mais que 10 letras");
		}
		...
		if (produto.getQtdEstoque() < 1) {
			throw new ExceptionMentoriaJava("O Produto deve conter no minimo 1 em estoque");
		}
		...
		if (produto.getImagens().size() > 6) {
			throw new ExceptionMentoriaJava("Deve ser informado no maximo 6 imagens para o produto");
		}
		...
		if (produto.getId() == null) {
			...
			for (int x = 0; x < produto.getImagens().size(); x++) {
				...
				/*Site que converte Base64 para Imagem: https://base64.guru/converter/decode/image*/
				/*Site que converte Imagem em Base64: https://products.aspose.app/imaging/pt/conversion/image-to-base64*/
		...
		if (produto.getAlertaQtdeEstoque() && produto.getQtdEstoque() < 1) {
			...
			html.append("<p>").append(produto.getId()).append("</p>");
